...
### Firmware ESP32 (Simulação via HTTP POST)
...
const char* serverUrl  = "http://SEU_IP:8080/sensores/leitura";
const char* jwtToken   = "Bearer SEU_TOKEN_AQUI";
...
void loop() {
    if (WiFi.status() == WL_CONNECTED) {
        ...
        // Simula leitura de sensor analógico
        float o2Level = 19.0 + (random(0, 50) / 10.0); // valor entre 19.0 e 24.0

        StaticJsonDocument<200> doc;
        doc["estufa_id"]    = 1;
        doc["tipo_sensor"]  = "OXIGENIO";
        doc["valor_leitura"] = o2Level;
        doc["unidade"]      = "PERCENTUAL";

        String payload;
        serializeJson(doc, payload);

        int httpCode = http.POST(payload);
        Serial.printf("Leitura enviada: %.2f%% | HTTP: %d\n", o2Level, httpCode);
        http.end();
    }
...
### Simulação sem Hardware (JSON Mockado)

Caso o ESP32 físico não esteja disponível, utilize o script Python de simulação:
...
SENSORS = [
    {"estufa_id": 1, "tipo_sensor": "OXIGENIO",    "unidade": "PERCENTUAL", "base": 21.0, "delta": 3.0},
    {"estufa_id": 1, "tipo_sensor": "UMIDADE",     "unidade": "PERCENTUAL", "base": 55.0, "delta": 20.0},
    {"estufa_id": 1, "tipo_sensor": "RADIACAO",    "unidade": "mSv/h",      "base": 0.8,  "delta": 1.5},
]

while True:
    for sensor in SENSORS:
        valor = sensor["base"] + random.uniform(-sensor["delta"], sensor["delta"])
        payload = {
        ...
        print(f"[{sensor['tipo_sensor']}] {valor:.2f} {sensor['unidade']} → HTTP {r.status_code}")
    time.sleep(5)
```
